...
class EditProfileView(APIView):
    ...
    def patch(self, request):
        ...
        if serializer.is_valid():
            ...
            email = serializer.validated_data['email']  # hanya untuk identifikasi, tidak diupdate
            profile_data = {}
            ...
            if profile_image:
                ...
                old_filepath = user_data.get("profile_image_path") if user_data else None
...
class UserProfileView(APIView):
    def get(self, request):
        ...
        if not email:
            return Response({"error": "Email harus disertakan di query parameter"}, status=status.HTTP_400_BAD_REQUEST)
